...
- The current Plan Mode slice has backend state, transcript events, desktop store plumbing, a persistent composer Plan Mode toggle, a lightweight desktop review card for confirm/cancel, and confirmed-plan context injected into model execution and session recovery; plan editing and stricter step-level execution constraints are not productized yet.
